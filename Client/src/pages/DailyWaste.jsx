...
import DriverApprovedRequest from "../Components/DriverApprovedRequest";
...
const AllDriverRequest = () => {
  ...
  const refetch = useCallback(async () => {
    try {
      const { data: refreshedData } = await customFetch.get(
        "/routePath/retriveRoutePath"
      );
  ...
  return (
    <allDriverRequestContext.Provider value={{ data, refetch }}>
      <div className="w-full h-screen overflow-x-auto">
        <DriverApprovedRequest /> {/* DriverRequest gets the same data */}
      </div>
